...
pub struct ParseRule {
    pub prefix: Option<fn(&mut Parser, &String, &mut Chunk, &mut scanner::Scanner)>,
    pub infix: Option<fn(&mut Parser, &String, &mut Chunk, &mut scanner::Scanner)>,
...
impl Parser {
    ...
    pub fn compile(&mut self, source: String, chunk: &mut Chunk) -> bool {
        ...
        self.expression(&source, chunk, &mut scanner);
    ...
    pub fn expression(&mut self, source: &String, chunk: &mut Chunk, scanner: &mut scanner::Scanner) {
        self.parse_precedence(source, chunk, Precedence::Assignment, scanner);
    }

    pub fn number(&mut self, source: &String, chunk: &mut Chunk, scanner: &mut scanner::Scanner) {
    ...
    pub fn grouping(&mut self, source: &String, chunk: &mut Chunk, scanner: &mut scanner::Scanner) {
        self.expression(source, chunk, scanner);
        self.consume(source, TokenType::RightParen, "Expect ')' after expression.", scanner);
    }

    pub fn unary(&mut self, source: &String, chunk: &mut Chunk, scanner: &mut scanner::Scanner) {
        let operator_type = self.previous.clone().token_type;
        self.parse_precedence(source, chunk,  Precedence::Unary, scanner);
        match operator_type {
    ...
    pub fn binary(&mut self, source: &String, chunk: &mut Chunk, scanner: &mut scanner::Scanner) {
        let operator_type = self.previous.clone().token_type;
        let rule = self.get_rule(&operator_type);
        self.parse_precedence(source, chunk, rule.precedence.clone().increase(), scanner);
        match operator_type {
    ...
    pub fn parse_precedence(&mut self, source: &String, chunk: &mut Chunk, precedence: Precedence, scanner: &mut scanner::Scanner) {
        self.advance(source, scanner);
        let prefix_rule = self.get_rule(&self.previous.token_type).prefix;
        if let Some(prefix) = prefix_rule {
            prefix(self, source, chunk, scanner);
        }
        ...
        while precedence <= self.get_rule(&self.current.token_type).precedence {
            ...
            if let Some(infix) = infix_rule {
                infix(self, source, chunk, scanner);
            }
        }
    }
}